...
class AdminRecommendationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def interactions(self, request):
        ...
        queryset = Interact.objects.select_related('user', 'product')
    ...
    def statistics(self, request):
        ...
        # User statistics
        total_users_with_interactions = Interact.objects.values('user').distinct().count()
        active_users = Interact.objects.filter(
            created_at__gte=date_from
        ).values('user').distinct().count()
        
        # Product statistics
        products_with_interactions = Interact.objects.values('product').distinct().count()
        ...
        return Response({
            ...
            'interactions': {
                ...
                'avg_per_user': round(total_interactions / total_users_with_interactions, 2) if total_users_with_interactions else 0
            },
            'users': {
            ...
            'products': {
                ...
                'coverage_percentage': round((products_with_interactions / total_products * 100), 2) if total_products else 0
            },
            'ratings': {
                'total': total_ratings,
